...
# 이 경로는 겹칠 수 없고, 서로 접할 수도 없다. 즉
# , 각 칸을 지나는 파이프는 하나이어야 한다.
#
# 원웅이 빵집의 모습이 주어졌을 때,
# 원웅이가 설치할 수 있는 가스관과 빵집을
# 연결하는 파이프라인의 최대 개수를 구하는 프로그램을 작성하시오.
#
# 입력
...
bakery = []
for i in range(R):
    bakery.append(list(input()))

print(bakery)
visit = []
# 파이프는 1로 표기
def connect_pipe(x:int, y:int):
    if bakery[x][y] == '.':
        visit[x][y] = '1'
    elif bakery[x][y] == 'x':
        visit[x][y] = 'x'
...
# .....
# ...x.
# 첫번째 열은 근처 가스관, 마지막 열은 원웅이 빵집 가스관

# 제일 위에서 시작
# 제일 오른쪽 대각선 위로 갈수 있는지 확인 >
# 막히면 오른쪽으로 갈 수 있는지 확인
# 갈 수 없다면 오른쪽 대각선 밑으로 갈 수 있는지 확인한다.
# 다 안되면 다음 줄로 이동
